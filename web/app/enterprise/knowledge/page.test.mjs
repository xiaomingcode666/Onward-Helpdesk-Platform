...
test("tenant default knowledge base supports document upload and management", () => {
  ...
  assert.match(source, /tenantKnowledgeSelected && tenantKnowledgeBaseID/)
  assert.match(source, /<KnowledgeRightPanel/)
...
test("knowledge base access grants are managed from the tenant knowledge scope", () => {
  ...
  assert.match(source, /handleOpenKnowledgeAccess/)
  assert.match(source, /canManageKnowledgeAccess/)
  assert.match(source, /session\?\.permissions\?\.includes\("knowledgeBase\.view"\)/)
  assert.match(source, /session\?\.permissions\?\.includes\("knowledgeBase\.update"\)/)
  assert.match(source, /tenantKnowledgeBaseID && canManageKnowledgeAccess/)
  assert.match(source, /enterpriseKnowledge\.access\.manage/)
  assert.match(source, /knowledgeAccessTeams\.map/)
  assert.doesNotMatch(source, /ProductModuleAccess/)
})
...
test("knowledge content column uses stable title and filename text styles", () => {
  ...
  assert.match(source, /<DataTable<ProductKnowledgeDocumentFile>/)
