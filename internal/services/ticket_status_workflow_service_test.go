...
func TestTicketStatusWorkflowPublishRuntimeAndRestore(t *testing.T) {
	...
	runCaseAction(t, db, ticket.ID, "acknowledge", "客服受理", op)
	runCaseAction(t, db, ticket.ID, "triage", "", op)
	require.NoError(t, repositories.TicketRepository.Updates(db, ticket.ID, map[string]any{"current_assignee_id": int64(902), "status": enums.TicketStatusPendingAssigneeAccept}))
	...
	require.Equal(t, op.UserID, closed.CaseOwnerID)
	...
	runCaseAction(t, db, fresh.ID, "acknowledge", "受理", op)
